...
extern int16_t accel_data[3];  // Stores the 16-bit signed accelerometer sensor output
extern int16_t gyro_data[3];   // Stores the 16-bit signed gyro sensor output
extern int16_t mag_data[3];    // Stores the 16-bit signed magnetometer sensor output

extern float f_accel_data[3]; 			// Bias corrections for accelerometer
extern float f_gyro_data[3];	// Bias corrections for gyro 
extern float f_mag_data[3];  // Bias corrections for mag 
extern float magCalibration[3];	// Factory mag calibration
...
extern int buffersize;     //Amount of readings used to average, make it higher to get more precision but sketch will be slower  (default:1000)
extern int acel_deadzone;     //Acelerometer error allowed, make it lower to get more precision, but sketch may not converge  (default:8)
extern int giro_deadzone;     //Giro error allowed, make it lower to get more precision, but sketch may not converge  (default:1)
extern int acel_step;     //For accel pih coefficient buffer / acel_step

extern int mean_ax,mean_ay,mean_az,mean_gx,mean_gy,mean_gz,state;

extern bool is_meansensors;
extern bool is_start_meansensors ;
extern bool is_first_offset;
extern int buff_ax , buff_ay , buff_az , buff_gx , buff_gy , buff_gz ;
extern int16_t ax_offset, ay_offset, az_offset, gx_offset, gy_offset, gz_offset;
extern DEV_IIC_PTR mpu_i2c_ptr;

void MPU9250_INIT(void);
void MPU9250_READ(void);

int32_t writeByte(uint8_t i2c_address , uint8_t reg_address , uint8_t data);
